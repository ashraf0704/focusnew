...
import { BookOpen, Plus, Check, Trash2, Sparkles, GraduationCap } from 'lucide-react';
...
export default function SubjectOnboardingModal({ isOpen, onClose, onSaveSubjects }: SubjectOnboardingModalProps) {
  ...
  const handleAddCustom = (e: React.FormEvent) => {
    ...
    if (customSubjects.includes(customInput.trim())) return;
    setCustomSubjects(prev => [...prev, customInput.trim()]);
    setCustomInput('');
  };
  ...
  return (
    <AnimatePresence>
      <div className="fixed inset-0 bg-[#4A4A3A]/40 backdrop-blur-md z-50 flex items-center justify-center p-4">
        <motion.div
          initial={{ opacity: 0, scale: 0.95, y: 20 }}
          animate={{ opacity: 1, scale: 1, y: 0 }}
          exit={{ opacity: 0, scale: 0.95, y: 20 }}
          className="bg-white w-full max-w-xl rounded-3xl border border-brand-outline shadow-2xl p-6 sm:p-8 flex flex-col max-h-[90vh] overflow-hidden"
        ...
        >
            ...
            <div className="pt-2 border-t border-brand-outline/60">
              ...
              <form onSubmit={handleAddCustom} className="flex gap-2">
                <input
                  ...
                  placeholder="e.g. Physics, Biology, Data Science..."
                  className="flex-1 bg-slate-50 border border-brand-outline rounded-xl py-2.5 px-3.5 text-xs text-brand-dark font-medium focus:outline-none focus:ring-1 focus:ring-brand-primary focus:bg-white transition"
                />
                <button
                  type="submit"
                  className="px-4 bg-brand-primary text-white rounded-xl text-xs font-bold flex items-center justify-center gap-1.5 transition hover:opacity-95 cursor-pointer"
                >
                  <Plus size={14} />
                  Add
                </button>
          ...
          <div className="pt-4 border-t border-brand-outline/60 mt-6 flex justify-end">
            <button
