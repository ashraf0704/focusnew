...
export function generateHTMLReports(results: TestCaseResult[]): void {
  ...
  // System Configuration Info Mock
  const deviceName = 'Nexus_6_API_34';
  const androidVersion = 'Android 14 (API 34)';
  const appVersion = '1.0.0 (Native Release Build)';
...
<head>
  <meta charset="UTF-8">
  <title>Focus Buddy - E2E Execution Report</title>
...
<body class="p-6">
  <div class="max-w-7xl mx-auto space-y-6">
    ...
    <header class="flex justify-between items-center pb-4 border-b border-stone-200">
      <div>
        <h1 class="text-3xl font-extrabold text-stone-800">Focus Buddy</h1>
        <p class="text-stone-500 text-sm">Android Appium E2E Automation Results</p>
      </div>
    ...
    <!-- Env Info -->
    <div class="card p-6 rounded-3xl shadow-xs grid grid-cols-1 md:grid-cols-3 gap-6">
      <div>
        <span class="block text-[10px] font-bold uppercase tracking-wider text-stone-400">Emulator Device</span>
        <span class="text-stone-700 font-bold">${deviceName}</span>
      </div>
      <div>
        <span class="block text-[10px] font-bold uppercase tracking-wider text-stone-400">Android Target</span>
        <span class="text-stone-700 font-bold">${androidVersion}</span>
      </div>
      <div>
        <span class="block text-[10px] font-bold uppercase tracking-wider text-stone-400">Binary Build version</span>
        <span class="text-stone-700 font-bold">${appVersion}</span>
      </div>
    </div>
    ...
    <div class="card p-6 rounded-3xl shadow-xs">
      <div class="flex justify-between items-center mb-6">
        <h2 class="text-lg font-black text-stone-800">Detailed Executed Test Cases</h2>
...
<head>
  <meta charset="UTF-8">
  <title>Focus Buddy - E2E Test Dashboard</title>
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <script src="https://cdn.tailwindcss.com"></script>
  <link href="https://fonts.googleapis.com/css2?family=Outfit:wght@300;400;600;800&display=swap" rel="stylesheet">
...
<body class="p-6">
  <div class="max-w-7xl mx-auto space-y-6">
    <header class="flex justify-between items-center pb-4 border-b border-stone-200">
      <div>
        <h1 class="text-3xl font-extrabold text-stone-800">Test Execution Metrics</h1>
        <p class="text-stone-500 text-sm">Dashboard and Analytical Charts</p>
      </div>
      <div>
        <a href="execution-report.html" class="px-4 py-2 border border-stone-200 rounded-xl text-xs font-bold text-stone-600 hover:bg-stone-50 bg-white transition shadow-xs">View Detailed List</a>
      </div>
    </header>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
      <div class="card p-6 rounded-3xl shadow-xs flex flex-col justify-between">
        <div>
          <h3 class="font-extrabold text-stone-800 text-base mb-4">Summary Totals</h3>
...
<head>
  <meta charset="UTF-8">
  <title>Focus Buddy - E2E Trends</title>
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <script src="https://cdn.tailwindcss.com"></script>
  <link href="https://fonts.googleapis.com/css2?family=Outfit:wght@300;400;600;800&display=swap" rel="stylesheet">
  <script src="https://cdn.jsdelivr.net/npm/chart.js"></script>
...
<body class="p-6">
  <div class="max-w-7xl mx-auto space-y-6">
    <header class="flex justify-between items-center pb-4 border-b border-stone-200">
      <div>
        <h1 class="text-3xl font-extrabold text-stone-800">Historical Trends</h1>
        <p class="text-stone-500 text-sm">Pass Rate Over Time</p>
      </div>
      <div>
        <a href="dashboard.html" class="px-4 py-2 border border-stone-200 rounded-xl text-xs font-bold text-stone-600 hover:bg-stone-50 bg-white transition shadow-xs">View Dashboard</a>
      </div>
    </header>
  ...
  <script>
    ...
    new Chart(ctx, {
      type: 'line',
      data: {
        labels: ['Build #1', 'Build #2', 'Build #3', 'Build #4', 'Build #5 (Latest)'],
        datasets: [{
          label: 'Pass Rate (%)',
          data: [95.0, 95.8, 96.2, 95.9, ${passRate}],
          borderColor: '#10B981',
