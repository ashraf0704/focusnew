...
export default function AIDoubtSolverScreen() {
  ...
  async function send() {
    ...
    await chat.send('groq', input || `Please explain ${attachmentName}`, subjectName, context);
    setInput('');
  }

  return (
    <BottomSheetView style={styles.screen}>
      <Text style={styles.title}>Groq Llama 70B Solver</Text>
      <Text style={styles.subtitle}>Single chatbot powered by llama-3.3-70b-versatile</Text>
      <AttachmentBar name={attachmentName} />
      <FlatList data={[...chat.messages.groq].reverse()} inverted keyExtractor={item => item.id} renderItem={({item}) => <ChatBubble message={item} />} style={styles.chat} />
      <View style={styles.inputRow}>
        <TextInput value={input} onChangeText={setInput} placeholder={`Ask Groq about ${subjectName}`} style={styles.input} />
